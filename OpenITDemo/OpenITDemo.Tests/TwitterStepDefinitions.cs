...
namespace OpenITDemo.Tests
{
    ...
    {
        private static readonly string _patternRand = @"{rand:(\d+)}";
        private static readonly string _patternDevice = @"{device}";
        ...
        private string TransformTweetText(string tweetText)
        {
            var match = Regex.Match(tweetText, _patternRand);
            var count = int.Parse(match.Groups[1].Value);
            var text = Regex.Replace(tweetText, _patternRand, Generator.Get(count));

            var deviceMatch = Regex.Match(tweetText, _patternDevice);
            text = Regex.Replace(text, _patternDevice, _context.Get<Device>("device").ToString());
